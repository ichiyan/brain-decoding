...
class StableDiffusionPriorNetwork(nn.Module):
    def __init__(
        self,
        dim,
        num_timesteps = None,
    ...
    def forward(
        ...
        *,
        ...
        # image_embed = image_embed.view(len(image_embed),-1,768)
        image_embed = image_embed.view(len(image_embed),-1,image_embed.shape[2])  #(bs, 1, 1024)
        # text_embed = text_embed.view(len(text_embed),-1,768)
        # brain_embed = brain_embed.view(len(brain_embed),-1,768)
        brain_embed = brain_embed.view(len(brain_embed),-1,image_embed.shape[2])
        # print(*image_embed.shape)
        # print(*image_embed.shape, image_embed.device, image_embed.dtype)
        ...
        # return brain_embed, time_embed, image_embed, learned_queries
