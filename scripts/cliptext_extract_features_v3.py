...
num_embed, num_features, num_test, num_train = 77, 768, len(caps[keys[1]]), len(caps[keys[0]])
num_features2 = 1280
...
with torch.no_grad():
    for data in keys:
        for i,annots in enumerate(caps[data]):
            ...
            print(prompt_embeds_list[0].shape, prompt_embeds_list[1].shape)

            clip_penultimate[data][i] = prompt_embeds_list[0].to('cpu').numpy().mean(0)
            openclip_penultimate[data][i] = prompt_embeds_list[1].to('cpu').numpy().mean(0)
            pooled[data][i] = pooled_prompt_embeds.to('cpu').numpy().mean(0)
        
        np.save('data/extracted_features/subj{:02d}/nsd_cliptext_penultimate_{}.npy'.format(sub, data), clip_penultimate[data])
        np.save('data/extracted_features/subj{:02d}/nsd_opencliptext_penultimate_{}.npy'.format(sub, data), openclip_penultimate[data])
