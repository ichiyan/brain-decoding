...
image_path = 'data/processed_data/subj{:02d}/nsd_train_depth_stim_sub{}.npy'.format(sub,sub)
train_images = batch_generator_external_images(data_path = image_path)

image_path = 'data/processed_data/subj{:02d}/nsd_test_depth_stim_sub{}.npy'.format(sub,sub)
...
for i,x in enumerate(testloader):
    ...
    with torch.no_grad():   
        ...
        print(stats[0]['z'].shape)

        im = recons[0]
        im = Image.fromarray(im)
        im = im.resize((512,512),resample=3)

        # im.save('results/test_stim_decoded.png')
        im.save('results/test_depth_stim_decoded.png')

        # orig_img = Image.fromarray(test_images[0])
        # orig_img = orig_img.resize((512,512),resample=3)
        # orig_img.save('results/test_orig.png')

        break

# data_input, target = preprocess_fn(test_images[0])
# activations = ema_vae.encoder.forward(data_input)
# px_z, stats = ema_vae.decoder.forward(activations, get_latents=True)
# recons = ema_vae.decoder.out_net.sample(px_z)
# print(type(recons))
...
#         print(len(stats))
#         print(stats[74].keys())
#         print(stats[74]['z'].shape)
#         print(stats[31]['z'].shape)
