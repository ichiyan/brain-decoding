...
train_clip = np.load('data/extracted_features/subj{:02d}/nsd_opencliptext_penultimate_train.npy'.format(sub))
test_clip = np.load('data/extracted_features/subj{:02d}/nsd_opencliptext_penultimate_test.npy'.format(sub))

# clip = np.concatenate((train_clip, test_clip), axis=0)
...
for i in range(num_embed):
    ...
    print(i,reg.score(test_fmri,test_clip[:,i]), 'alpha: %f' % reg.alpha_)

np.save('data/predicted_features/subj{:02d}/nsd_opencliptext_penultimate_predtest_nsdgeneral.npy'.format(sub),pred_clip)
...
with open('data/regression_weights/subj{:02d}/opencliptext_penultimate_regression_weights.pkl'.format(sub),"wb") as f:
