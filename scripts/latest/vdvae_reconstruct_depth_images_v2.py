...
pred_latents = np.load('data/predicted_features/subj{:02d}/nsd_vdvae_depth_nsdgeneral_pred_sub{}_{}l_alpha100k.npy'.format(sub,sub,num_layers))
ref_latent = stats
...
for i in range(int(np.ceil(len(test_images)/batch_size))):
  ...
  for j in range(len(sample_from_latent)):
      ...
      im.save('results/vdvae/subj{:02d}/depth_stim_{}l/{}.png'.format(sub,num_layers,i*batch_size+j))
