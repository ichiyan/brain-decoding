...
nsd_features = np.load('data/extracted_features/subj{:02d}/nsd_lvae_features.npz'.format(sub))
train_latents = torch.from_numpy(nsd_features['train_latents']).flatten(start_dim=1).numpy()
test_latents = torch.from_numpy(nsd_features['test_latents']).flatten(start_dim=1).numpy()
