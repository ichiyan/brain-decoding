...
train_clip = np.load('data/extracted_features/subj{:02d}/nsd_cliptext_train_v2.npy'.format(sub))
test_clip = np.load('data/extracted_features/subj{:02d}/nsd_cliptext_test_v2.npy'.format(sub))
...
for i in range(num_embed):
    ...
    # v3 - clip embeddings are not standardized
...
# np.save('data/predicted_features/subj{:02d}/nsd_cliptext_predtest_nsdgeneral_v2.npy'.format(sub),pred_clip)
np.save('data/predicted_features/subj{:02d}/nsd_cliptext_predtest_nsdgeneral_v3.npy'.format(sub),pred_clip)
...
# with open('data/regression_weights/subj{:02d}/cliptext_regression_weights_v2.pkl'.format(sub),"wb") as f:
#   pickle.dump(datadict,f)

with open('data/regression_weights/subj{:02d}/cliptext_regression_weights_v3.pkl'.format(sub),"wb") as f:
  pickle.dump(datadict,f)